...
- Implement `.edit-hotzone`, `.edit-toggle` labeled `编辑`, an `导出 HTML` control, and a small `InlineEditor` using `contenteditable`, `localStorage`, `E` to toggle when not inside editable text, and `Cmd/Ctrl+S` to save. Give the controls stable `data-editor-control` attributes (`toggle`, `export`, `reset`) and the export button `id="exportHtml"`, so tests locate controls without depending on UI copy.
...
- Save on every `input` event while editing (not only on exit or shortcut) and add a `beforeunload` fallback save, so closing the tab mid-edit never loses work.
- Sanitize pasted, dropped, and saved markup with a zero-dependency whitelist: keep only `b/strong/i/em/u/br/a[href]/span`, unwrap other tags, strip all attributes except `http(s)`/`mailto` hrefs, and drop comments. This keeps Word/web paste, dragged markup, and markup payloads from breaking the layout.
- While editing, intercept Enter: single-line roles such as headings must not insert line breaks; block roles (`p/li/blockquote/td/div`) insert `<br>` via `insertLineBreak` so the browser never splits the element.
- Keep shortcut guards unified: `E` toggles only when focus is outside `input, textarea, select, [contenteditable="true"]`; `Esc` exits editing once IME composition has ended; navigation shortcuts ignore events from editable nodes.
- Mark placeholder copy (待补充 etc.) with `data-placeholder="true"`, and before export count unedited placeholders and confirm with the user so obvious draft copy is not shipped silently.
- Back up the initial `innerHTML` once under `<storage-key>:original` and offer a `恢复初始` control that restores it after confirmation.
- While editing, clicking `a[data-edit-id]` or `img[data-edit-id]` opens a small panel to edit `href`/`src`; persist attributes in the same edit object under `@attr:<id>:<attribute>` keys.
- Keep the control reachable on touch devices: under `@media (hover: none)` it stays visible, and tapping the hotzone toggles it.
- Localize the small editor UI from `<html lang>` (中文/English) while keeping the data contract identical.
...
- Make `导出 HTML` serialize the current edited DOM into a new standalone `.html` download so edits survive across browsers, profiles, devices, and `file://` locations. Clone the DOM, remove editing state, set editable nodes back to `contenteditable="false"`, clear runtime-generated controls such as slide dots and editor panels, and preserve all embedded CSS/JavaScript. Embed the edit snapshot as `window.__EXPORTED_EDITS__` (the hero template uses `__HERO_EXPORTED_EDITS__`) so the exported file renders identically anywhere, and inject a capture-phase resource-error listener that shows a non-blocking banner when styles or images fail to load.
- Namespace exported-file `localStorage` with a unique `data-edit-version` on `<html>` while keeping the original unversioned key backward-compatible. This prevents stale edits from the original path from overwriting the exported markup when a user replaces the source file. When regenerating a page over the same file path, bump `data-edit-version` too, so old edits and the `:original` snapshot (which is keyed by version) do not leak into the new version.
- In exported files, place the embedded edit snapshot script in `<head>` so it executes before the editor script in `<body>`; otherwise stale same-path `localStorage` would load first and override the exported markup.
- Verify with the bundled suites: `npm run test:editor` (E toggle with input guard, paste sanitize, save/reload, reset, touch visibility) and `npm run test:export` (export round trip on every bundled template; also accepts explicit files, e.g. `node scripts/test-export-html.mjs <generated.html>`).
