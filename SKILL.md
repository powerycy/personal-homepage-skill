...
- Sanitize pasted and saved markup with a zero-dependency whitelist: keep only `b/strong/i/em/u/br/a[href]/span`, unwrap other tags, strip all attributes except `http(s)`/`mailto` hrefs, and drop comments. This keeps Word/web paste and markup payloads from breaking the layout.
